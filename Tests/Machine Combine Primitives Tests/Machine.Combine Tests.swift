...
// `store.insert({ ... } as @Sendable (T1, T2) -> U)`, hit only under `-O`
// / release builds. A concrete function-typed overload absorbs the
// closure at the call site so the outer generic `insert<Value: Sendable>`
// is never specialized directly against an inline-cast closure type. See
// `swift-institute/Experiments/silgen-sendable-typed-throws-closure-cast/`
// for the analogous (SILGen, not release-optimizer) sibling crash.
...
extension Machine.Capture.Store where Mode == Machine.Capture.Mode.Reference {
    fileprivate mutating func insert<In1: Sendable, In2: Sendable, Out: Sendable>(
        _ fn: @Sendable @escaping (In1, In2) -> Out
    ) -> Machine.Capture.ID<@Sendable (In1, In2) -> Out> {
